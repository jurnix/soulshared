...
module SHR_arrayUtils_mod
  ...
  interface initArrayRange
    module procedure initArrayRange_rsp
    module procedure initArrayRange_rdp
    module procedure initArrayRange_int
  end interface
...
contains
  ...
  function initArrayRange_rsp(cstart, cend, csteps) result (newArray)
    ...
    character(len=*) , parameter :: SNAME = "initArrayRange_rsp"
...
!        write(*,*) "initRealArrayRange:: error(is negative)= ", cstart, cend, csteps
  ...
  end function initArrayRange_rsp


  function initArrayRange_rdp(cstart, cend, csteps) result (newArray)
    !< create a new array generating values starting from 'start' to 'end'
    !< in 'steps'. 
    !< input: initRealArrayRange(89.5, -89.5, -0.5) -> 89.5, 89, ... to ...,-89,-89.5
    !< input: initRealArrayRange(-89.5, 89.5, 0.5) -> -89.5,-89, ... to ..., 89, 89.5
    !<  if cstart is bigger than cend, then csteps must be negative
    !<     cstart is smaller than cend, then csteps must be positive
    real(kind=dp), intent(in) :: cstart
    real(kind=dp), intent(in) :: cend
    real(kind=dp), intent(in) :: csteps

    real(kind=dp), allocatable :: newArray(:)
    ...
    integer :: i
    character(len=200) :: tmp
    character(len=*) , parameter :: SNAME = "initArrayRange_rdp"
...
!        write(*,*) "initRealArrayRange:: error(is negative)= ", cstart, cend, csteps
  ...
  end function initArrayRange_rdp


  function initArrayRange_int(cstart, cend, csteps) result (newArray)
    !< create a new array generating values starting from 'start' to 'end'
    !< in 'steps'. 
    !< input: initRealArrayRange(89.5, -89.5, -0.5) -> 89.5, 89, ... to ...,-89,-89.5
    !< input: initRealArrayRange(-89.5, 89.5, 0.5) -> -89.5,-89, ... to ..., 89, 89.5
    ...
    integer, intent(in) :: csteps

    integer, allocatable :: newArray(:)
    ...
    integer :: i
    character(len=200) :: tmp
    character(len=*) , parameter :: SNAME = "initArrayRange_int"
...
!        write(*,*) "initRealArrayRange:: error(is negative)= ", cstart, cend, csteps
  ...
  end function initArrayRange_int
