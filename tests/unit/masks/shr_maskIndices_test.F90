...
contains
  ...
  subroutine testMaskIndices2d(self)
    ...
    call m2 % init(4,1,3,0)
    call self % assert(.true., "m2 % init(4,1,3,0) = T")
    ...
    call self % assert(m2 == mother, "m2(4,1,3,0) .eq. mOther(cols(3,0), rows(4,1)) = T")

    !< different attributes
    call m2 % init(4,1,3,0)
    call mother % init(4,1,3,1)
    call self % assert(.not. (m2 == mother), "m2(4,1,3,0) .eq. mOther(4,1,3,1) = T")

    !procedure :: toString => toString_maskIndices_2d
    call m2 % init(4,1,3,0)
    call self % assert(m2 % toString() == "[(3:0), (4:1)]", &
            "m2(4,1,3,0) % toString() = T")
    ...
    call self % assert(mother % getRow() == rows, &
        "mOther(4,1,3,0) % getRow() .eq. rows(4,1) = T")
    ...
    call self % assert(mother % getCol() == cols, &
        "mOther(4,1,3,0) % getRow() .eq. cols(3,0) = T")
