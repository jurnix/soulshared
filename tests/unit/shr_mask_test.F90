...
contains
  ...
  subroutine defineTestCases(self)
    ...
    mMatrixIndices = shr_mask_calc_groups_indices(mask2d)

    call self % assert(size(mMatrixIndices) == 5, &
    "shr_mask_calc_groups_indices_2d(TTFFTT,T,T,T,FFFFT) size .eq. 5 = T")
    ...
    call self % assert(all(mMatrixIndices == expMatrixIndices), &
          "shr_mask_calc_groups_indices_2d(TTFFTT,T,T,T,FFFFT) .eq. (...) = T")
